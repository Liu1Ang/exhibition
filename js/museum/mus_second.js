$(function () {
    //选项卡显示对应选项
    var index = sessionStorage.getItem('fy_mus_index')?sessionStorage.getItem('fy_mus_index'):0;
    if(index) {
    ...
    var newsId = sessionStorage.getItem('fy_mus_id')?sessionStorage.getItem('fy_mus_id'):52;
    init();
    var num = 0;  //提示esc退出
    var scrolltop = 0;  //滚动条定位
...
//              	console.log(this)
                    ...
                    html_list += '<li id='+ this.id +' style="width:700px;"><h3>'+ this.title +'</h3>' +
                    ...
                    html1 += "<div class='white-panel'><img class='lazy thumb' src=http://www.foyuanzhilu.cn" + this.img_url + " id=" + this.id + "></div>";
                });
                if(newsId == 52){
                    $('#grid1').html(html);
                }
                if(newsId == 54){
                    $('#gallery-wrapper').html(html1);
                }
...
//              	console.log(this)
        ...
        img_details:function () {
            $('.product-wrap-item img').on('click',function () {
